...
## Day 030
Spent over 2 hours implementing the FastAPI backend for the AI Engineering Bootcamp project #1, integrating the existing AI inference pipeline through a `/predict` endpoint with file validation, Pydantic response schemas, and the two-stage potato-leaf detection → disease-classification workflow. Tested the API locally through Swagger UI with both valid potato-leaf images and invalid/non-potato uploads, confirming appropriate predictions, confidence scores, explanations, recommendations, and rejection handling. Set up the backend dependencies, committed the completed backend structure and implementation on a feature branch, pushed it to GitHub, and created a pull request that was subsequently merged into main.

## Day 031
Spent several hours continuing the backend integration and deployment preparation for the AI Engineering Bootcamp project #1. Reviewed the existing AI inference pipeline in greater depth to understand how the two-stage potato leaf detection and disease classification workflow was exposed to the FastAPI backend, including the detector, disease classifier, confidence handling, explanations, recommendations, and rejection of non-potato images. Finalized the backend architecture and merged the FastAPI implementation into `main`, then added Cross-Origin Resource Sharing (CORS) support for local frontend integration and subsequently made the allowed origins configurable through an environment variable following an automated code review by Copilot. Set up the project for cloud deployment on *Render*, connected the GitHub repository, configured the Python web service, build command, Uvicorn start command, and deployment settings. The first deployment attempt failed due to dependency compatibility issues in `requirements.txt`, particularly around the Python 3.14 environment and SciPy/PyTorch versions, which led to reviewing the locally installed package versions and aligning the deployment dependencies with the working local environment.

## Day 032
Today, I spent some time debugging the initial failed Render deployment of the FastAPI backend. After correcting the dependency versions, the application successfully built and reached the deployment stage, but the Render Free instance repeatedly failed at runtime because of its *512 MB memory limit*. Investigated the deployment logs and traced an initial memory spike to an unnecessary download of the pretrained ImageNet ResNet-18 weights during inference, despite the project already containing the trained model checkpoints. Traced this through the inference pipeline into the shared `create_model()` function in `ai/models/model.py`, which was hard-coded to initialize `ResNet-18` with `ResNet18_Weights.DEFAULT`. Modified the model factory to make pretrained initialization configurable while preserving pretrained weights for training, and updated the inference pipeline to instantiate the disease classifier with `pretrained=False` before loading the existing trained checkpoint. Verified the modified inference module locally and pushed the deployment fix to main, after which Render confirmed that the unnecessary pretrained-weight download was eliminated. However, the service still exceeded the 512 MB memory limit during runtime, showing that the complete PyTorch inference process itself likely requires more memory than the available Render Free instance. Paused there for now due to limited time and fast-approaching deadline. Considering Streamlit for quick deployment. 